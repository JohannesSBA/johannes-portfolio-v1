...
const LightRays: React.FC<LightRaysProps> = ({
  ...
  useEffect(() => {
    ...
    const initializeWebGL = async () => {
      ...
      // Ensure a single background canvas without removing React children
      ...
      if (!existingCanvas) {
        ...
        containerRef.current.prepend(gl.canvas);
      } else if (existingCanvas !== gl.canvas) {
        // Replace any stray canvas with the new one
        existingCanvas.replaceWith(gl.canvas);
        gl.canvas.style.position = "absolute";
        gl.canvas.style.top = "0";
        gl.canvas.style.left = "0";
        gl.canvas.style.width = "100%";
...
void main() {
  ...
  return (
    <div
      ref={containerRef}
      className={`min-h-screen min-w-screen top-0 overflow-hidden relative ${className}`.trim()}
    >
      <div className="min-h-screen min-w-screen relative z-[10] pointer-events-none flex items-center justify-center"></div>
    </div>
  );
};
